...
def educate():
    ...
    test = pd.read_csv("../news/068270.csv", encoding="UTF-8")[['Title']]
    ...
    if not os.path.isfile("../NLP_model2/saved_model.pb"):
    ...
    # model.save("../NLP_model2")
    model = load_model("../NLP_model2")
    print("NLP model save Completed!")
    ...
    print(sample)
    sample.to_csv("../educate_method_result.csv", encoding="UTF-8")
...
def predict(news):
    ...
    test = pd.read_csv("../news/068270.csv", encoding="UTF-8")[["Date", 'Title']]
    # test = test[:int(len(test)*0.1)]
...
if __name__ == "__main__":
    educate()
    predict("a")
    sent = pd.read_csv("../analysis.csv", encoding="UTF-8")[['Title', 'Label']]
